      ...
      if (
        ...
        for (let i = 0; i < data.profileUnshared.length; i++) {
          if (data.profileUnshared[i]._id == type) {
            ...
            obj.BusinessHoursSwitch = (data.profileUnshared[i].businessHoursSwitch);
...
const monthView = async (profile,type) => {
  try {
    const config = {
      headers: {
...
const fetchConnectionData = async (profile,type) => {
  try {
    const config = {
      headers: {
        Authorization: "Bearer " + cookies().get("jwt_token").value,
...
export default async function Dashboards({params}) {
  const templateData = await fetchProfile(params.userName);
  const data = (templateData.Type != ""? (await getData(params.userName,templateData.Type)):{});
  const monthData = (templateData.Type != ""? (await monthView(params.userName,templateData.Type)):[]);
  const connections = (templateData.Type != ""? (await fetchConnectionData(params.userName,templateData.Type)):[]);
  const profileData = (templateData.Type != ""? (await fetchId(params.userName,templateData.Type)):{});
  const toggleData = (templateData.Type != ""? (await fetchreview(params.userName,templateData.Type)):{});
  
  return (
    <Dashboard data={data} templateData={templateData} monthData={monthData} connections={connections} profileData={profileData} toggleData={toggleData} userName={params.userName}/>
  )
}
