...
export default function ProfileViews() {
    ...
    async function handleSubmit(){
        ...
        `${serverUrl}/analytics/profileViews`,
        {
            startDate: stDate,
            endDate: edDate,
        },
    ...
    const transformDataForLineChart = async (viewsData) => {
        ...
        for (
            ...
            // newIpList.forEach((element)=>result[element]=0);
            // viewsData.forEach(element => {
            //   if(element.date == formattedDate){
            //     if(result[element.ip] == undefined) result[element.ip] = element.viewCount;
            //     else result[element.ip] += element.viewCount;
            //   }  
            // });
            viewsData.forEach(element => {
              if(element.date == formattedDate){
                result.refreshCount += element.viewCount;
              }  
            });
            result.viewCount = viewsData.filter(element => (element.date == formattedDate)).length;
      ...
      >
          <div className="flex flex-col w-full m-auto items-center gap-4 ">
            ...
            <div className="flex flex-nowrap text-center text-base my-2">
              <p>Profile Views between selected range: </p>
              <div className="text-center mx-2">
                  {loading? <p className="text-base inline-block">Loading</p>:views}
              </div>
            </div>
        </div>
          <div
