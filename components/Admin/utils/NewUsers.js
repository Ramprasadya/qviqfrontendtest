...
export default function NewUsers() {
    ...
    const [newUsers,setNewUsers] = useState({basicCount:0,starterCount:0,proCount:0});
    ...
    async function handleSubmit(){
        ...
        `${serverUrl}/analytics/getNewUsers`,
        {
            startDate: stDate,
            endDate: edDate,
            interval: interval,
        ...
        let obj = {basicCount:0,starterCount:0,proCount:0};
        Array.from(data).forEach((doc,idx)=>{
            obj.basicCount += doc.basicCount;
            obj.starterCount += doc.starterCount;
            obj.proCount += doc.proCount;
        })
        setNewUsers(obj);
        setLoading(false);
        ...
        if (active && payload && payload.length) {
          return (
            ...
            >
              ...
              <p className="flex justify-between mt-0.5 text-xs text-[#1A1A1A]">
                <span>Pro Users</span>
                <span>{payload[2] !== undefined ? payload[2].value : "00"}</span>
              </p>
            </div>
          );
        }
    ...
    const transformDataForLineChart = async (viewsData) => {
        ...
        for (
            ...
            if(interval == "daily"){
                const ele = viewsData.filter(element => (element._id.year == date.getFullYear() && element._id.month == date.getMonth()+1 && element._id.dayofmonth == date.getDate()))[0];
                result.basicCount = ele? ele.basicCount:0;
                result.starterCount = ele? ele.starterCount:0;
                result.proCount = ele? ele.proCount:0;
            } else if(interval == "monthly"){
                const ele = viewsData.filter(element => (element._id.year == date.getFullYear() && element._id.month == date.getMonth()+1))[0]
                result.basicCount = ele? ele.basicCount:0;
                result.starterCount = ele? ele.starterCount:0;
                result.proCount = ele? ele.proCount:0;
            }else{
                const ele = viewsData.filter(element => (element._id.year == date.getFullYear()))[0];
                result.basicCount = ele? ele.basicCount:0;
                result.starterCount = ele? ele.starterCount:0;
                result.proCount = ele? ele.proCount:0;
            } 
  ...
  return (
    <>
      <div
        ...
        <div className="flex w-full items-center p-3">
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <div className="flex m-auto items-center justify-center ">
              ...
              value={interval}
              onChange={(e) => {setLoading(true);setInterval(e.target.value)}}>
        ...
        <div className="flex flex-nowrap text-center text-base my-2">
            <p>New Users between selected range: </p>
            <div className="text-center mx-2">
                {loading? <p className="text-base inline-block">Loading</p>:(newUsers.basicCount+newUsers.starterCount+newUsers.proCount)}
            </div>
        </div>
        <div className="flex flex-nowrap items-center justify-center">
            <div className="flex flex-nowrap text-center text-base my-2 border border-gray p-1">
                <p>Basic Users: </p>
                <div className="text-center mx-2">
                    {loading? <p className="text-base inline-block">Loading</p>:(newUsers.basicCount)}
                </div>
            </div>
            <div className="flex flex-nowrap text-center text-base my-2 border border-gray p-1">
                <p>Starter Users: </p>
                <div className="text-center mx-2">
                    {loading? <p className="text-base inline-block">Loading</p>:(newUsers.starterCount)}
                </div>
            </div>
            <div className="flex flex-nowrap text-center text-base my-2 border border-gray p-1">
                <p>Pro Users: </p>
                <div className="text-center mx-2">
                    {loading? <p className="text-base inline-block">Loading</p>:(newUsers.proCount)}
                </div>
            </div>
        </div>
      </div>
