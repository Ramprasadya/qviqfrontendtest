...
function TapopGoogleQr(props) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      fetch(`${serverUrl}/person/gupdatedimage/${profile}/${id}`, {
        method: "PUT",
        body: formData,
        profile: profile,
      });
  ...
  const changeColour = (e) => {
    axios.put(`${serverUrl}/person/gupdatedColour/${profile}/${id}`, {
      colour: e,
...
const getQRcode = async () => {
  try {
    ...
    // Change 'sget' to 'gget'
    const res = await axios.get(`${serverUrl}/person/gget/${profile}`)
    ...
    setColorIndex(pcolour.indexOf(data.colour));
    setQrvalue(data.Qr);
    ...
    if (data.image && data.image !== "") {
      setImage(data.image);
      setNewQrCodeLogoUrl(data.image);
      setQRlogo(data.image);
    }
    
    // Append QR codes if both image and colour exist
