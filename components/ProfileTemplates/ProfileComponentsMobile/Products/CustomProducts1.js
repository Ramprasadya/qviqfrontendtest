    ...
    <div className="w-full">
      ...
      <div className="relative">
        <div className="overflow-scroll" ref={productRef}>
          <div className="flex flex-row gap-4 pb-[15px]">
            {data.map((item, index) => {
              return (
                <div className={`${style.card}`} key={index}>
                  ...
                  <h1 className={`${style.description}`}>{item.productDescription}</h1>
                  <h1 className={`${style.price}`}>₹ {item.productPrice}</h1>
                  <div className={`${item.productButton ? 'visible' : 'hidden'} h-[56px]`}>
                    <CustomButton
                      text={item.label}
