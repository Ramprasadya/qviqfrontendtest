...
const Hero2 = (props) => {
  ...
  return (
    <div
      className={`max-w-[720px] w-[720px] min-h-[387px] flex flex-col items-center relative ${style.div}`}
    >
      <div className="absolute left-1/2 -translate-x-1/2 top-[-70px]">
        <img
          src={`${data.profilePic}`}
          className={`min-w-[140px] min-h-[140px] max-w-[140px] max-h-[140px] ${style.profilePic}`}
          alt="profilepic"
        />
      </div>
      <div className="w-full pt-[110px] px-[60px] text-center">
        <p className={`mb-[5px] ${style.fullName}`}>{data.fullName}</p>
        <p className={`mb-[5px] ${style.jobTitle}`}>{data.jobTitle}</p>
        <p className={`mb-[5px] ${style.jobTitle}`}>{data.companyName}</p>
        <p className={`mb-[30px] ${style.jobDescription}`}>
          {data.jobDescription}
        </p>
        <div className="flex justify-center gap-4">
          {!dummy
            ? data.mobileNumber !== "" && (
                <p
                  className={`inline-flex ${style.border}  justify-center gap-1 hover:cursor-pointer md:w-[152px] w-[90px]  xsm:w-[105px] h-[47px] rounded-[8px] bg-[rgba(255,255,255,0.16)] ${style.jobTitle}`}
          ...
          {!dummy
            ? data.email !== "" && (
                <p
                  className={`inline-flex ${style.border} justify-center gap-1 hover:cursor-pointer md:w-[152px]  xsm:w-[105px] w-[90px] h-[47px] rounded-[8px] bg-[rgba(255,255,255,0.16)] ${style.jobTitle}`}
                ...
                >
                  <span className="flex items-center font-[600] leading-normal">
                    <svg
                      ...
                      viewBox="0 0 20 20"
                      fill="none"
                      className={`mt-[3px] h-5 w-5  ${style.color} `}
