...
function UsernamePage({ searchParams }) {
  ...
  const sendTemplateMessage = async () => {
    ...
    try {
      const response = await fetch(`${serverUrl}/whatsapp/loginMessage`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  return (
    <>
      <div className="relative w-full h-screen pt-[62px] smd:pt-[96px] pb-[25px] flex flex-col justify-start items-center smd:bg-[transparent] bg-white font-poppins z-0">
        <Navbar usedIn="hideLogin" background="transparent" />

        <div
          className="hidden md:flex flex-row items-center gap-[8px] cursor-pointer active:scale-[90%] absolute top-0 left-0 pl-[50px] lg:pl-[80px] pt-[62px] smd:pt-[96px] transition-[300ms]"
